        ...
        Body()(
            Article(data_signals="{'srConnected': false, 'connectionInvalid': false}")(
                Fieldset(role="group")(
                    ...
                    # ),
                    Button(
                        data_text="$srConnected ? 'Connected' : 'Connect'",
...
async def connect(request, session: dict):
    ...
    if not sr:
        return DatastarResponse(
            SSE.patch_signals({"srConnected": False, "connectionInvalid": "true"}),
        )
    user_id = session.setdefault("user_id", str(uuid.uuid4()))
    user_data = all_user_data[user_id]
    if "sr" not in user_data or user_data["sr"] != sr or "cap" not in user_data:
        try:
            ...
            list_container_patches = [
                SSE.patch_elements(
                    el, selector="#containers", mode=ElementPatchMode.APPEND
                )
            ...
            return DatastarResponse(
                itertools.chain(
                    [
                        SSE.patch_signals(
                            {"srConnected": True, "connectionInvalid": "false"}
                        ),
                        SSE.patch_elements(
                            selector="#no_container_placeholder",
    ...
    try:
        ...
        for entry in entries:
            css_id = get_css_id_from_user_data(
                user_id, f"{container_c_id}.e_{entry.key}"
            )
            ...
            rows.append(
                Tr(id=f"delete_{css_id}", data_signals=json.dumps(sigs))(
                    ...
                    Td()(
                        Button(
                            "Edit",
                            data_on_click=f"@get('/containers/{container_c_id}/entries/{entry_e_key}')",
                        ),
                        Button(
                            "Delete",
                            data_on_click=f"@delete('/containers/{container_c_id}/entries/{entry_e_key}')",
                        ),
                    ),
                )
            )
...
async def get_entry(request, session, container_c_id: str, entry_e_key: str):
    ...
    user_id = session.get("user_id", None)
    container = get_container_from_user_data(user_id, container_c_id)
    if not container or "idForEdit" not in sigs:
        return DatastarResponse()
...
async def get_entry(
    request, session, container_c_id: str, entry_e_key: str, value_type: str
):
    signals = await read_signals(request)
...
def storage_input_field(
    ...
    if val_type == "boolValue":
        sigs = {
            container_c_id: {
                entry_e_key: {
                    "boolValueInvalid": False,
                    "boolValue": stor_val.boolValue,
                }
            }
        }
        return Input(
